...
function PlaylistArtwork({ playlist, className = "" }: { playlist: Playlist; className?: string }) {
  return <Artwork track={{ id: playlist.id, title: playlist.name, artist: "Wave Tune playlist", album: playlist.name, duration: 0, artwork: playlist.artwork, accent: playlist.accent, source: "catalog" }} className={className} />;
}
...
function HomeView({ tracks, profile, onNavigate }: { tracks: Track[]; profile?: SpotifyProfile; onNavigate: (view: View) => void }) {
  ...
  const recent = profile ? tracks.filter((track) => track.source === "spotify").slice(0, 5) : tracks.filter((track) => recentIds.has(track.id)).slice(0, 5);
...
function PlaylistView({ playlist, onBack }: { playlist: Playlist; onBack: () => void }) {
  ...
  const tracks = playlist.tracks?.length ? playlist.tracks : playlist.trackIds.map((id) => catalogTracks.find((track) => track.id === id)).filter(Boolean) as Track[];
...
export default function App() {
  ...
  const allTracks = useMemo(() => [...spotify.tracks, ...spotify.likedTracks, ...library, ...catalogTracks].filter((track, index, list) => list.findIndex((item) => item.id === track.id) === index), [spotify.tracks, spotify.likedTracks, library]);
