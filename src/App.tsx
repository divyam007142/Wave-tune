...
type SpotifyState = { profile?: SpotifyProfile; tracks: Track[]; recentTracks: Track[]; likedTracks: Track[]; playlists: Playlist[]; loading: boolean; error: string | null };
...
function PlaylistArtwork({ playlist, className = "" }: { playlist: Playlist; className?: string }) {
  return <Artwork track={{ id: playlist.id, title: playlist.name, artist: "Spotify playlist", album: playlist.name, duration: 0, artwork: playlist.artwork, accent: playlist.accent, source: "spotify" }} className={className} />;
}
...
function SourcePill({ source }: { source: Track["source"] }) {
  return <span className={`source-pill source-${source}`}>{source === "spotify" ? "Spotify" : "On this device"}</span>;
}
...
function Sidebar({ activeView, onNavigate, collapsed, setCollapsed, onImport, profile, spotifyPlaylists, onConnect, onDisconnect }: { activeView: View; onNavigate: (view: View) => void; collapsed: boolean; setCollapsed: (value: boolean) => void; onImport: () => void; profile?: SpotifyProfile; spotifyPlaylists: Playlist[]; onConnect: () => void; onDisconnect: () => void }) {
  return <aside className={`sidebar ${collapsed ? "is-collapsed" : ""}`}>
    ...
    <nav className="sidebar-nav" aria-label="Primary navigation"><span className="nav-label">Recommend</span>{[{ id: "home" as View, label: "For you", icon: Home }, { id: "library" as View, label: "Library", icon: Library }, { id: "search" as View, label: "Radio Station", icon: Radio }, { id: "search" as View, label: "Music Video", icon: Play }].map(({ id, label, icon: Icon }) => <button key={label} className={`nav-item ${activeView === id && label === "For you" ? "is-active" : ""}`} onClick={() => onNavigate(id)} title={collapsed ? label : undefined}><Icon size={15} /><span>{label}</span></button>)}<span className="nav-label sidebar-sub-label">My music</span>{navItems.filter((item) => ["liked", "recent", "playlists"].includes(item.id)).map(({ id, label, icon: Icon }) => <button key={id} className={`nav-item ${activeView === id ? "is-active" : ""}`} onClick={() => onNavigate(id)} title={collapsed ? label : undefined}><Icon size={15} /><span>{label === "Liked songs" ? "Liked Song" : label === "Recently played" ? "Recent" : "Playlists"}</span></button>)}</nav>
     <div className="sidebar-playlists"><div className="sidebar-playlist-head"><span className="nav-label">Playlists</span><IconButton label="Open playlists" onClick={() => onNavigate("playlists")}><Plus size={16} /></IconButton></div>{spotifyPlaylists.slice(0, 2).map((playlist) => <button className="sidebar-playlist" key={playlist.id} onClick={() => onNavigate("playlists")}><span className="playlist-dot" style={{ backgroundImage: playlist.artwork ? `url(${playlist.artwork})` : undefined }} /><span>{playlist.name}</span></button>)}</div>
...
function TopBar({ profile, onSearch, onSettings, onConnect, onMenu }: { profile?: SpotifyProfile; onSearch: () => void; onSettings: () => void; onConnect: () => void; onMenu: () => void }) {
  const canConnect = !profile && spotifyService.isConfigured;
...
function HomeView({ tracks, profile, onNavigate }: { tracks: Track[]; profile?: SpotifyProfile; onNavigate: (view: View) => void }) {
  ...
  const queuePreview = tracks.slice(0, 5);
   return <div className="page home-page reference-home">{tracks.length ? <><section className="reference-release-section"><div className="reference-section-heading"><h1>Releases for You</h1><span>{profile ? `Curated for ${profile.name}` : "Connect Spotify to load your music"}</span></div><div className="reference-release-layout"><div className="reference-featured-cover"><Artwork track={featured} priority /><button className="reference-cover-play" aria-label={`Play ${featured.title}`} onClick={() => featured && playTrack(featured, tracks)}><Play size={16} fill="currentColor" /></button></div><div className="reference-track-list">{queuePreview.map((track, index) => <ReferenceTrackRow key={track.id} track={track} index={index} context={queuePreview} />)}</div></div></section><section className="reference-recent-section"><div className="reference-section-heading"><h2>Recently Played</h2><button className="reference-see-all" onClick={() => onNavigate("recent")}>See all</button></div><div className="card-row">{recent.map((track) => <TrackCard key={track.id} track={track} context={recent} />)}</div></section></> : <div className="empty-state"><Music2 size={28} /><h2>Connect Spotify to start listening</h2><p>Wave Tune uses Spotify for your profile, playlists, track metadata, artwork, and durations.</p></div>}</div>;
}
...
function FullPlayer({ onClose }: { onClose: () => void }) {
  ...
  if (!currentTrack) return <motion.div className="full-player-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.section className="full-player"><div className="full-player-head"><IconButton label="Close player" onClick={onClose}><ChevronDown size={21} /></IconButton><span>NOW PLAYING</span><span /></div><div className="empty-state"><Music2 size={28} /><h2>Nothing playing</h2><p>Connect Spotify and choose a track to open the player.</p></div></motion.section></motion.div>;
...
function LibraryView({ onImport, spotifyTracks }: { onImport: () => void; spotifyTracks: Track[] }) {
  const { library } = usePlayer();
  const items = [...library, ...spotifyTracks].filter((track, index, list) => list.findIndex((item) => item.id === track.id) === index);
...
function PlaylistView({ playlist, onBack }: { playlist: Playlist; onBack: () => void }) {
  ...
  return <div className="page playlist-page"><button className="back-button" onClick={onBack}><ArrowLeft size={15} /> All playlists</button><section className="playlist-hero"><PlaylistArtwork playlist={playlist} /><div><span className="eyebrow">PLAYLIST</span><h1>{playlist.name}</h1><p>{playlist.description}</p><span className="playlist-meta">{tracks.length ? `${tracks.length} songs · Curated by you` : "Spotify metadata · Audio remains separate"}</span><div className="playlist-actions">{tracks[0] && <button className="main-play small" onClick={() => playTrack(tracks[0], tracks)}><Play size={15} fill="currentColor" /> Play</button>}<IconButton label="Shuffle playlist"><Shuffle size={17} /></IconButton><IconButton label="More playlist options"><MoreHorizontal size={18} /></IconButton></div></div></section>{tracks.length ? <div className="track-list">{tracks.map((track, index) => <TrackRow key={track.id} track={track} index={index} context={tracks} />)}</div> : <div className="empty-state compact-empty"><Radio size={23} /><h2>Spotify playlist metadata</h2><p>Playlist track loading is ready for the authorized Spotify service. Playback remains local or Spotify Connect only.</p></div>}</div>;
}
...
export default function App() {
  ...
  const [spotify, setSpotify] = useState<SpotifyState>({ tracks: [], recentTracks: [], likedTracks: [], playlists: [], loading: spotifyService.connected, error: null });
  ...
  const disconnectSpotify = () => { spotifyService.disconnect(); setSpotify({ tracks: [], recentTracks: [], likedTracks: [], playlists: [], loading: false, error: null }); };
  ...
  const onImport = () => fileInputRef.current?.click();
   const allTracks = useMemo(() => [...spotify.tracks, ...spotify.likedTracks, ...library].filter((track, index, list) => list.findIndex((item) => item.id === track.id) === index), [spotify.tracks, spotify.likedTracks, library]);
  const recentTracks = useMemo(() => [...spotify.recentTracks, ...recentlyPlayed.map((id) => allTracks.find((track) => track.id === id)).filter(Boolean) as Track[]].filter((track, index, list) => list.findIndex((item) => item.id === track.id) === index), [allTracks, recentlyPlayed, spotify.recentTracks]);
  const likedTracks = useMemo(() => [...allTracks.filter((track) => likedIds.includes(track.id)), ...spotify.likedTracks].filter((track, index, list) => list.findIndex((item) => item.id === track.id) === index), [allTracks, likedIds, spotify.likedTracks]);
   const displayPlaylists = useMemo(() => spotify.playlists, [spotify.playlists]);
  const page = selectedPlaylist && activeView === "playlist" ? <PlaylistView playlist={selectedPlaylist} onBack={() => { setSelectedPlaylist(null); setActiveView("playlists"); }} /> :
    ...
    activeView === "library" ? <LibraryView onImport={onImport} spotifyTracks={spotify.likedTracks} /> :
